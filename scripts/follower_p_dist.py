...
class Follower:
    ...
    def __init__(self):
        ...
        self.command_queue = []
        self.backup_command = Follower.TWIST_DEFAULT
        self.current_command = Twist()
        self.backup_control = False
        self.img_msg = None
        self.pose_initialized = 0.
    ...
    def queue_command(self, twist_command):
        dist_target = self.distance_traveled + COMMAND_DIST_DELAY
        self.command_queue.insert(0, (dist_target, twist_command))
        print(f"{self.current_command.linear.x:.2f} m/s, {np.rad2deg(self.current_command.angular.z):.1f} deg/s")

    def execute_command(self):
        twist_command = None
        if len(self.command_queue) and self.current_command.linear.x > 0:
            if self.distance_traveled >= self.command_queue[-1][0]:
                self.backup_control = False
                dist_target, twist_command = self.command_queue.pop()
                print("Queue size:", len(self.command_queue))
            elif self.backup_control:
                twist_command = Follower.TWIST_DEFAULT
        else:
            self.backup_control = True
            twist_command = self.backup_command
            #print(twist_command.angular.z)
        ...
        if twist_command is not None:
            self.current_command = smooth_exp_twist(self.current_command, twist_command,
                ...
                alpha_angular_rising=ANGLE_RAMP_FACTOR,
                alpha_angular_falling=ANGLE_RAMP_FACTOR)
            #print(f"{self.current_command.linear.x:.2f} m/s, {np.rad2deg(self.current_command.angular.z):.1f} deg/s")
    ...
    def process_image(self, msg):
        ...
        # Find our path
        centerpoint = fit_road(left_edges_xy, right_edges_xy, x_mid, max_dist=DIST_LOOKAHEAD, draw_img=img_overhead)
        ...
        if centered:
            ...
            v = SPEED_NORMAL if np.abs(turning_radius_signed) < STRAIGHTAWAY_MIN_TURN_RADIUS else SPEED_HIGH
            w = v / turning_radius_signed - road_offs / 2.5
        else:
